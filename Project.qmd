...
```{r}
votes_103 <- read.csv("votes_time_series.csv") %>%
...
votes_pca_rec <- recipe(~ ., data = votes_103) %>%
  step_pca(starts_with("v"), id = "pca") %>%
  prep()

variance_explained <- votes_pca_rec %>%
...
#the first component explained 0.38% of variance and the first 5 together explained 0.66%
...
plot_party <- ggplot(joined, aes(x = PC1, y = PC2, color = party)) +
  geom_point() +
  labs(title = "Scatterplot of PCA by Party", x = "PC1", y = "PC2") +
  theme_minimal()

plot_region <- ggplot(joined, aes(x = PC1, y = PC2, color = Region)) +
  geom_point() +
